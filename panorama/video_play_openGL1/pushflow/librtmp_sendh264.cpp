 ...
 * @成功则返回 1 , 失败则返回0
 */
int SendVideoSpsPps(unsigned char *pps, int pps_len, unsigned char * sps, int sps_len, int m_nTimeStamp)
{
...
int RTMP264_Send(int (*read_buffer)(unsigned char *buf, int buf_size))
{
    ...
    int ret;
    static NaluUnit naluUnit;
    static int width = 0,height = 0, fps=0;
    static unsigned int tick;
    static unsigned int tick_gap;
    int bKeyframe;
...
extern struct vpu_h264_out_buffer vpu_out_buffer;
int read_buffer1(unsigned char *buf, int buf_size )
{
    return vpu_buffer_remove(vpu_out_buffer, (char *)buf, buf_size);
//    if(vpu_out_buffer.index < vpu_out_buffer.out_size){
//        return read_h264_raw_data(buf, buf_size);
//    }else{
//        return -1;
//    }
}
...
{
    int left = vpu_out_buffer.out_size - vpu_out_buffer.index;
    if(left > buf_size){
        memcpy(buf, (vpu_out_buffer.out_buf + vpu_out_buffer.index), buf_size);
        vpu_out_buffer.index += buf_size;
        return buf_size;
    }else{
        memcpy(buf, (vpu_out_buffer.out_buf + vpu_out_buffer.index), left);
        vpu_out_buffer.index += left;
        return left;
    }
}
