...
v4l_start_capturing(void)
{
	...
	for (i = 0; i < TEST_BUFFER_NUM; i++) {
        ...
        cap_buffers[i].start = (unsigned char *)mmap (NULL, cap_buffers[i].length,
...
v4l_capture_setup(struct encode *enc, int width, int height, int fps)
{
	...
	info_msg("enc->cmdl->video_node_capture=== %d\n",enc->cmdl->video_node_capture);
...
//	info_msg("sensor chip is %s\n", 
    ...
    if ((ioctl(cap_fd,VIDIOC_QUERYCAP,&cap)) < 0) {
        info_msg("VIDIOC_QUERYCAP failed \n");
        return -1;
    }

	info_msg("Capability Informations:\n");
    info_msg(" driver: %s\n", cap.driver);
    info_msg(" card: %s\n", cap.card);
    info_msg(" bus_info: %s\n", cap.bus_info);
    info_msg(" version: %08X\n", cap.version);
    info_msg(" capabilities: %08X\n", cap.capabilities);
...
//		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_NV12;
    ...
    fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;//YUY422
    ...
    // Print Stream Format
    info_msg("Stream Format Informations:\n");
    info_msg(" type: %d\n", fmt.type);
    info_msg(" width: %d\n", fmt.fmt.pix.width);
    info_msg(" height: %d\n", fmt.fmt.pix.height);
	info_msg(" Image size = %d\n", fmt.fmt.pix.sizeimage);
	enc->sizeimage=fmt.fmt.pix.sizeimage;
	enc->width=fmt.fmt.pix.width;
	enc->height=fmt.fmt.pix.height;
	enc->bytesperline=fmt.fmt.pix.bytesperline;
