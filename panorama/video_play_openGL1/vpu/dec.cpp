...
static int mjpg_read_chunk_till_soi(struct decode *dec)
{
	...
	while (1)
	{
		if (dec->mjpg_rd_ptr == dec->mjpg_wr_ptr) {
            nread = vpu_read(dec->cmdl, (char *)buf + dec->mjpg_wr_ptr,
					MJPG_FILL_SIZE);
			if (nread < 0) {
				/* error */
				err_msg("nread %d < 0\n", nread);
		...
		int *eos, int *fill_end_bs)
{
	...
	*eos = 0;
	...
	if ( (target_addr + size) > bs_va_endaddr) {
        ...
        nread = vpu_read(cmd, (char *)target_addr, room);
		if (nread <= 0) {
			/* EOF or error */
			if (nread < 0) {
				if (nread == -EAGAIN)
			...
			*eos = 1;
			...
			/* read the remaining */
			space = nread;
            nread = vpu_read(cmd, (char *)bs_va_startaddr,
					(size - room));
			if (nread <= 0) {
				/* EOF or error */
				if (nread < 0) {
        ...
        nread = vpu_read(cmd, (char *)target_addr, size);
		if (nread <= 0) {
			/* EOF or error */
			if (nread < 0) {
				if (nread == -EAGAIN)
	...
	while (1) {
		...
		if (cpu_is_mx6x() && (dec->cmdl->format == STD_MJPG)) {
			if (dec->mjpgLineBufferMode) {
				...
				if (mjpgReadChunk) {
                    ret = (RetCode)mjpg_read_chunk(dec);
					if (ret < 0)
						return ret;
					else if (ret == 0)
						break;
...
decoder_allocate_framebuffer(struct decode *dec)
{
    ...
    fb = dec->fb = (FrameBuffer *)calloc(totalfb, sizeof(FrameBuffer));
	if (fb == NULL) {
		err_msg("Failed to allocate fb\n");
		return -1;
	}

    pfbpool = dec->pfbpool = (struct frame_buf **)calloc(totalfb, sizeof(struct frame_buf *));
	if (pfbpool == NULL) {
		...
		if (deblock_en == 0) {
			...
			if (!cpu_is_mx27()) {
				mvcol_md = dec->mvcol_memdesc =
                    (vpu_mem_desc *)calloc(totalfb, sizeof(vpu_mem_desc));
			}
			...
			for (i = 0; i < totalfb; i++) {
				...
				if (!cpu_is_mx27()) {
                    ...
                    ret = (RetCode)IOGetPhyMem(&mvcol_md[i]);
					if (ret) {
...
decoder_parse(struct decode *dec)
{
	...
	if (dec->userData.enable) {
        ...
        dec->userData.addr = (Uint8 *)malloc(SIZE_USER_BUF);
		if (!dec->userData.addr)
	...
	if (dec->frameBufStat.enable) {
        dec->frameBufStat.addr = (Uint8 *)malloc(initinfo.reportBufSize.frameBufStatBufSize);
		if (!dec->frameBufStat.addr)
			err_msg("malloc_error\n");
	}
	if (dec->mbInfo.enable) {
        dec->mbInfo.addr = (Uint8 *)malloc(initinfo.reportBufSize.mbInfoBufSize);
		if (!dec->mbInfo.addr)
			err_msg("malloc_error\n");
	}
	if (dec->mvInfo.enable) {
        dec->mvInfo.addr = (Uint8 *)malloc(initinfo.reportBufSize.mvInfoBufSize);
		if (!dec->mvInfo.addr)
			err_msg("malloc_error\n");
	}
...
decoder_open(struct decode *dec)
{
    ...
    oparam.bitstreamFormat = (CodStd)dec->cmdl->format;
	...
	ret = vpu_DecOpen(&handle, &oparam);//打开解码器实例
	if (ret != RETCODE_SUCCESS) {
...
decode_test(void *arg)
{
	...
	if (cpu_is_mx6x() && (dec->cmdl->format == STD_MJPG)
			&& dec->mjpgLineBufferMode) {
        dec->mjpg_cached_bsbuf = (u8 *)malloc(STREAM_BUF_SIZE);
