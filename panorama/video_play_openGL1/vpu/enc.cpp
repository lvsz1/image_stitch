...
// vpu 数据传递时同步
extern sem_t sem_full; // 缓存中存在拼接好的图像
extern sem_t sem_empty;//缓存中不存在拼接好的图像

//转码后的图像数据
extern uchar *DST_MATCH_RGB;
...
enc_readbs_reset_buffer(struct encode *enc, PhysicalAddress paBsBufAddr, int bsBufsize)
{
    ...
    return vpu_write(enc->cmdl, (char *)vbuf, bsBufsize);
}
		...
		int defaultsize)
{
	...
	if (space > 0) {
		...
		if ( (target_addr + space) > bs_va_endaddr) {
            ...
            vpu_write(cmd, (char *)target_addr, room);
            vpu_write(cmd, (char *)bs_va_startaddr,(space - room));
		} else {
            vpu_write(cmd, (char *)target_addr, space);
		}
...
put_mp4header:
		...
		if (enc->ringBufferEnable == 0 ) {
            ret = (RetCode)enc_readbs_reset_buffer(enc, enchdr_param.buf, enchdr_param.size);
			if (ret < 0)
				return -1;
		}
		...
		if (cpu_is_mx6x()) {
			...
			if (enchdr_param.pParaSet) {
                ...
                vpu_write(enc->cmdl, (char *)enchdr_param.pParaSet, enchdr_param.size);
...
encoder_allocate_framebuffer(struct encode *enc)
{
	...
	if (enc->cmdl->src_scheme == PATH_V4L2) {
        ret = (RetCode)v4l_capture_setup(enc, enc->src_picwidth, enc->src_picheight, enc->cmdl->fps);
		if (ret < 0) {
			goto err1;
		}
	} else {
...
//    }
    ...
    sem_wait(&sem_full);
    memcpy((void *)y_addr, DST_MATCH_RGB, img_size);
//    y_addr += img_size;
    ret = img_size;
    sem_post(&sem_empty);
    printf("vpu thread\n");
...
int   YUV422To420(unsigned char *yuv422, unsigned char *yuv420,struct encode *enc)
{     
	...
	// according to format
	isize = task.input.paddr =enc->sizeimage;
...
encoder_start(struct encode *enc)
{
	...
	if (!(cpu_is_mx6x() && (enc->cmdl->format == STD_MJPG))) {
        ret = (RetCode)encoder_fill_headers(enc);
		if (ret) {
	...
	if (src_scheme == PATH_V4L2) {
        ret = (RetCode)v4l_start_capturing();
		if (ret < 0) {
			return -1;
		}
...
//	FILE * fd_y_file = 0;
//    if ((fd_y_file = fopen("capture.yuv", "wb")) ==NULL)
//	{
...
//	}
	...
	// Input image size and format
    task.input.width    = 640;//640;
    task.input.height   = 480;//480;
    task.input.format   = v4l2_fourcc('Y', 'U', 'Y', 'V');
	...
	* fmt_to_bpp(task.input.format)/8;
	...
	// input.paddr contains the amount needed
	// this value will be replaced with physical address on success
    ret = (RetCode)ioctl(fd_ipu, IPU_ALLOC, &task.input.paddr);
	if (ret < 0) {
		printf("ioctl IPU_ALLOC fail: (errno = %d)\n", errno);
		return  -1;
	}
	...
	// Create memory map and obtain the allocated memory virtual address
    inbuf = (unsigned char *)mmap(0, isize, PROT_READ | PROT_WRITE,
	MAP_SHARED, fd_ipu, task.input.paddr);
	if (!inbuf) {
		printf("mmap fail\n");
		return  -1;
    ...
    ret = (RetCode)ioctl(fd_ipu, IPU_ALLOC, &task.output.paddr);
	if (ret < 0) {
		printf("ioctl IPU_ALLOC fail\n");
		return  -1;
	}
...
//    char *yv12_buffer = (char *)malloc(isize);
	...
	while (1) {
		...
		if (src_scheme == PATH_V4L2) {
            ...
            ret = (RetCode)v4l_get_capture_data(&v4l2_buf);
			if (ret < 0) {	
				goto err2;		
			}
            ...
            ret = (RetCode)ioctl(fd_ipu, IPU_QUEUE_TASK, &task);
			if (ret < 0) {		
            ...
            ret = (RetCode)read_from_file(enc);
            if (ret <= 0)
                break;
		}
		...
		while (vpu_IsBusy()) {
			...
			if (enc->ringBufferEnable == 1) {
                ret = (RetCode)enc_readbs_ring_buffer(handle, enc->cmdl,
...
//        //key frame ?
        if(enc->cmdl->dst_scheme == PATH_MV){
            enc->cmdl->av_ctx->key_frame = outinfo.picType;
            printf("outinfo.picType: %d\n", outinfo.picType);
        }

		if (enc->ringBufferEnable == 0) {
            ret = (RetCode)enc_readbs_reset_buffer(enc, outinfo.bitstreamBuffer, outinfo.bitstreamSize);
			if (ret < 0) {
    ...
    {
        //Write file trailer
        av_write_trailer(enc->cmdl->av_ctx->pFormatCtx);
    }
...
encoder_configure(struct encode *enc)
{
	...
	if (enc->cmdl->rot_en) {
		...
		vpu_EncGiveCommand(handle, SET_ROTATION_ANGLE,
					&enc->cmdl->rot_angle);
        mirror = (MirrorDirection)enc->cmdl->mirror;
...
encoder_open(struct encode *enc)
{
    ...
    encop.bitstreamFormat = (CodStd)enc->cmdl->format;
