...
decode(void)
{
    ...
    ret = (RetCode)v4l_put_data(dec, outinfo.indexFrameDisplay, V4L2_FIELD_ANY, 30);
	if (ret) {
...
encoder_fill_headers(void)
{
	...
	if (enc->cmdl->format == STD_MPEG4) {
		if (cpu_is_mx27()) {
            ...
            ret = (RetCode)dec_fill_bsbuffer((char *)vbuf, enchdr_param.size);
			if (ret < 0)
				return -1;
		...
		if (cpu_is_mx6x()) {
			...
			if (enchdr_param.pParaSet) {
                ...
                ret = (RetCode)dec_fill_bsbuffer((char *)enchdr_param.pParaSet, enchdr_param.size);
...
encode(void)
{
	...
	if (cpu_is_mx6x() && (dec->cmdl->format == STD_MJPG)) {
        ret = (RetCode)encoder_fill_headers();
		if (ret) {
	...
	vbuf = (enc->virt_bsbuf_addr + outinfo.bitstreamBuffer
				- enc->phy_bsbuf_addr);
    ret = (RetCode)dec_fill_bsbuffer((char *)vbuf, outinfo.bitstreamSize);
	if (ret < 0) {
