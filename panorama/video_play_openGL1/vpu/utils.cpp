...
vpu_write(struct cmd_line *cmd, char *buf, int n)
{
	int fd = cmd->dst_fd;
    frame_count ++;
//    memcpy(current_ptr, buf, n);
//    current_ptr += n;
    printf("frame_count: %d\n", frame_count);

	if (cmd->dst_scheme == PATH_NET) {
		return udp_send(cmd, fd, buf, n);
    }else if(cmd->dst_scheme == PATH_LIBRTMP){

        vpu_buffer_add(vpu_out_buffer, buf, n);
...
//        vpu_out_buffer.index = 0;
        ...
        return 0;
    }else if(cmd->dst_scheme == PATH_MV){
        AVPacket pkt;
        av_init_packet(&pkt);
        pkt.data = (uint8_t *)buf;
        pkt.pts = cmd->av_ctx->frame_index;
        pkt.size = n;
        AVRational time_base = { 1, 1000 };
        AVRational r_framerate1 = { 50, 2 };
...
//        Duration between 2 frames (us)
        ...
        pkt.pos = -1;
        av_packet_rescale_ts(&pkt, cmd->av_ctx->pCodecCtx->time_base, cmd->av_ctx->pFormatCtx->streams[0]->time_base);
        pkt.stream_index = 0;

//        if(0 == cmd->av_ctx->key_frame){
//            pkt.flags |= AV_PKT_FLAG_KEY;
//            printf("it is the key frame\n");
//        }

//        pkt.pts = (cmd->av_ctx->frame_index) * (90000/25);
//        pkt.dts = pkt.pts;
        pkt.stream_index = 0;

        if (av_write_frame(cmd->av_ctx->pFormatCtx, &pkt) < 0) {
...
//check_params(&input_arg[0].cmd, input_arg[0].mode) == 0)
int
