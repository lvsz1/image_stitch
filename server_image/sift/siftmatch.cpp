...
//如果定义 DISPLAY, 则opencv建立窗口显示图片，否则不显示
//#define DISPLAY 1
...
SiftMatch::SiftMatch(IplImage *image1, IplImage *image2)
{
    // deep copy
...
SiftMatch::~SiftMatch()
{
    ...
    if(xformed_proc != NULL)
        cvReleaseImage(&xformed_proc);
}

IplImage *SiftMatch::match()
{
    detect_features();
    match_features();
    match_image();
    return cvCloneImage(xformed_proc);
}

IplImage* SiftMatch::get_image1()
{
    return cvCloneImage(img1);
}

IplImage* SiftMatch::get_image2()
{
    return cvCloneImage(img2);
}

IplImage *SiftMatch::get_match_image()
{
    return cvCloneImage(xformed_proc);
}
